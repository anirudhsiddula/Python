...
def get_word_score(word, n):
    ...
    prod = 7*len(word)-3*(n-len(word))
    return max(letter_sum*prod,letter_sum)
...
def update_hand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    relevant_letters = hand.copy()
    
    for i in word.lower():
        if relevant_letters.get(i, 0) > 0:
            relevant_letters[i] -= 1
...
def is_valid_word(word, hand, word_list):
    ...
    def replace_wildcard(word):
        ...
        for vowel in VOWELS:
            ...
            new_word = word[0:wild_index] + vowel            
            if wild_index < len(word) - 1:
                new_word += word[wild_index + 1:]
            wordlst.append(new_word.lower())
            
        return wordlst
            
    
    if '*' in word:
        wild_word_not_present = True
        flag = False
        repl_wild = replace_wildcard(word)
        for w in repl_wild:
            if w.lower() not in word_list:
                flag = flag or False
            else:
                flag = flag or True
        return flag
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        # Display the hand
        print('Current Hand: {}'.format(display_hand(hand)), end='')
        ...
        if word == '!!':
            ...
            tot_score += points_earned
            print('\"{}\" earned {} points. Total: {} points'.format(word, points_earned, tot_score))
        else:
...
def substitute_hand(hand, letter):
    ...
    if not letter.isalpha() or letter not in hand_copy.keys():
        return hand_copy
    while True:
        substitute_letter = random.choice(VOWELS if letter in VOWELS else CONSONANTS)
        if substitute_letter not in hand.keys():
            hand_copy[substitute_letter] = hand_copy[letter]
            break
    return hand_copy
